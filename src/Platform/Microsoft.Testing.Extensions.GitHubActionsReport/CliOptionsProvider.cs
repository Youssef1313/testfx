...
internal sealed class CliOptionsProvider(GitHubTestReporterExtension extension) : ICommandLineOptionsProvider
{
    ...
    public Task<ValidationResult> ValidateCommandLineOptionsAsync(ICommandLineOptions commandLineOptions)
    {
        ...
        // If --report-github isn't set, we error if the sub options are set.
        return commandLineOptions.IsOptionSet(ReportGitHubSummaryOption)
            || commandLineOptions.IsOptionSet(ReportGitHubTitleOption)
            || commandLineOptions.IsOptionSet(ReportGitHubMessageOption)
            ? ValidationResult.InvalidTask("The options 'report-github-summary', 'report-github-title', and 'report-github-message' can only be used if 'report-github' is set.")
    ...
    public Task<ValidationResult> ValidateOptionArgumentsAsync(CommandLineOption commandOption, string[] arguments)
    {
        if (commandOption.Name == ReportGitHubSummaryOption)
        {
            ...
            {
                if (arguments[i] is not ReportGitHubSummaryArguments.IncludePassedTests
                    and not ReportGitHubSummaryArguments.IncludeSkippedTests)
                {
                    return ValidationResult.InvalidTask($"The option '{ReportGitHubSummaryOption}' can only have the arguments '{ReportGitHubSummaryArguments.IncludePassedTests}' and '{ReportGitHubSummaryArguments.IncludeSkippedTests}'.");
                }
            }
        }
